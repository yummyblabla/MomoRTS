...
let listener = function (clients, sessionInfo, index, data) {
    ...
    switch (data.type) {
        ...
        case "begin-login": {
            if (!server_1.server.validateProperties(data, ["username"])) {
                return;
            }
            mongo.getSaltForUser(data.username, (salt) => {
        ...
        case "login": {
            if (!server_1.server.validateProperties(data, ["username", "value"])) {
                return;
            }
            mongo.checkLogin(data.username, session.loginToken, data.value, () => {
                ...
                mongo.getDetails(session.username, (details) => {
        ...
        case "create-account": {
            if (!server_1.server.validateProperties(data, ["username", "storedPW", "email"])) {
                return;
            }
            if (!session.newSalt) {
                return;
        ...
        // Verify the account
        case "verify-account": {
            if (!server_1.server.validateProperties(data, ["username", "activationCode"])) {
                return;
            }
...
//# sourceMappingURL=LoginListeners.js.map